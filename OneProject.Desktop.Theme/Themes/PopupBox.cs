...
public partial class PopupBox : ContentControl
{
    ...
    public static readonly DependencyProperty CornerRadiusProperty = DependencyProperty.Register(
        nameof(CornerRadius), typeof(double), typeof(PopupBox), new PropertyMetadata(default(double)));
    ...
    public static readonly DependencyProperty PopupElevationProperty = DependencyProperty.Register(
        nameof(PopupElevation), typeof(Elevation), typeof(PopupBox), new PropertyMetadata(Elevation.Se0));
    ...
    {
        if(PopupMode is PopupBoxPopupMode.MouseOverEager
            or PopupBoxPopupMode.MouseOver)
        {
            Close();
        }
    ...
    {
        double x, y;
        var offsetX = PopupHorizontalOffset;
        var offsetY = PopupVerticalOffset;

        var popupSizeTransformed = new Size(DpiHelper.TransformFromDeviceX(this, popupSize.Width), DpiHelper.TransformFromDeviceY(this, popupSize.Height));
        var targetSizeTransformed = new Size(DpiHelper.TransformFromDeviceX(this, targetSize.Width), DpiHelper.TransformFromDeviceY(this, targetSize.Height));
        ...
        var xTransformed = DpiHelper.TransformToDeviceX(this, x);
        var yTransformed = DpiHelper.TransformToDeviceY(this, y);
    ...
    {
        if(sender is not PopupBox popupBox || Equals(Mouse.Captured, popupBox))
        {
            return;
        }
        ...
        else
        {
            if((Mouse.Captured as DependencyObject).GetVisualAncestry().Contains(popupBox._popup?.Child))
            {
                ...
                if(!popupBox.IsPopupOpen || Mouse.Captured is not null || GetCapture() != IntPtr.Zero)
                {
                    return;
                }
    ...
    {
        if(PopupMode == PopupBoxPopupMode.Click || !IsPopupOpen)
        {
            return;
        }
