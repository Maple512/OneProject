...
internal class Screen
{
        ...
        get
        {
            if(_screens == null)
            {
                if(_multiMonitorSupport)
                {
                    ...
                    {
                        var temp = new Screen[closure.Screens.Count];
    ...
    public static Screen? PrimaryScreen
    {
        get
        {
            if(_multiMonitorSupport)
            {
                foreach(var screen in AllScreens ?? Enumerable.Empty<Screen>())
                {
                    if(screen.Primary)
                    {
                        return screen;
    ...
    public Rect WorkingArea
    {
        get
        {
            ...
            if(_currentDesktopChangedCount != DesktopChangedCount)
            {
                ...
                if(!_multiMonitorSupport || _hmonitor == PRIMARY_MONITOR)
                {
                    // Single monitor system
    ...
    public static Screen FromRect(Rect rect)
    {
        if(_multiMonitorSupport)
        {
            var rc = new NativeMethods.RECT(rect);
